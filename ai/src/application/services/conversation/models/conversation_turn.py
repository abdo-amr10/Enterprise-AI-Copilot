...
class ConversationTurn:
    ...
    def from_raw(raw: dict[str, Any]) -> Optional["ConversationTurn"]:
        """Parses one `{"role": "turn", ...}` dict supporting both camelCase and snake_case."""
        if raw.get("role") != "turn":
            return None
        turn_id = raw.get("turn_id") or raw.get("turnId")
        user_question = raw.get("user_question") or raw.get("userQuestion") or raw.get("question")
        if not turn_id or not user_question:
            return None
        return ConversationTurn(
            turn_id=str(turn_id),
            user_question=str(user_question),
            resolved_question=raw.get("resolved_question") or raw.get("resolvedQuestion"),
            generated_sql=raw.get("generated_sql") or raw.get("generatedSql") or raw.get("sql"),
            execution_result_summary=(
                raw.get("execution_result_summary")
                or raw.get("executionResultSummary")
                or raw.get("text_summary")
                or raw.get("textSummary")
            ),
            execution_status=(
                raw.get("execution_status")
                or raw.get("executionStatus")
                or raw.get("status")
            ),
