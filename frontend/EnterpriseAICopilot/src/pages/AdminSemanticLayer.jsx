...
const sources = [
  ['⌘', 'Schema Definition', 'JSON or YAML', 'erp_semantic_schema.json', '14 KB'],
  ['▣', 'Documentation', 'Markdown or PDF', 'analytics_docs.md', '42 KB'],
  ['⌁', 'Business Glossary', 'CSV or Excel', 'enterprise_terms_mapping.csv', '8 KB'],
  ['◉', 'Sample Data', 'CSV, Parquet, or JSONL', 'sample_cohort_1000.parquet', '8.12 MB'],
]
...
function UploadSources({ onCancel }) {
  ...
  return <section className="source-upload">
    ...
    <div className="source-grid">{sources.map(([icon, title, type, file, size]) => <article className="source-file-card"><div className="source-file-top"><div><b className="source-type-icon">{icon}</b><strong>{title}</strong><small>{type}</small></div><button type="button" aria-label={`Remove ${title}`}>×</button></div><div className="source-file-ready"><span>▤</span><div><b>{file}</b><small>{size} · Ready</small></div><i>✓</i></div></article>)}</div>
    <div className="source-footer"><button type="button" onClick={onCancel}>Cancel</button><button className="primary" type="button" onClick={() => { setLoading(true); window.setTimeout(() => setLoading(false), 750) }}>{loading ? 'Loading sources…' : '◉  Load Data Sources'}</button></div>
  </section>
}
