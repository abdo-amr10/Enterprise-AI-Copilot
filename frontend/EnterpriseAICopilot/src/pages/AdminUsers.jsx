...
export default function AdminUsers() {
  ...
  return <main className="admin-shell"><AdminSidebar active="users" /><section className="admin-main"><AdminTopBar title="Users" description="Manage user accounts and access levels." /><div className="admin-tabs">{[['users', 'Users'], ['add', 'Add User'], ['role', 'Access'], ['password', 'Password'], ['delete', 'Remove User']].map(([key, label]) => <button key={key} className={tab === key ? 'active' : ''} type="button" onClick={() => setTab(key)}>{label}</button>)}</div>{panels[tab]}</section></main>
}
